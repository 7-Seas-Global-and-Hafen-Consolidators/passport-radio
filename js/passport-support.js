(() => {
  ...
  const DISMISS_KEY='passport-adblock-dismissed-v7';
  ...
  const resourceProbe=()=>new Promise(resolve=>{const script=document.createElement('script');let settled=false;let timeout;const finish=blocked=>{if(settled)return;settled=true;clearTimeout(timeout);script.remove();resolve(blocked);};script.async=true;script.src='/ads.js';script.onload=()=>finish(false);script.onerror=()=>finish(true);document.head.appendChild(script);timeout=setTimeout(()=>finish(true),2200);});
  const maybeShowAdblockModal=async()=>{if(dismissed())return;try{await Promise.all([baitProbe(),resourceProbe()]);}catch(_e){}showAdblockModal();};

  const boot=()=>{ensureCss();removeLegacy();let section=document.getElementById('apoie');if(section){section.className='passport-support-global';section.innerHTML=markup();section.dataset.passportSupport='global';}else{section=document.createElement('section');section.id='apoie';section.className='passport-support-global';section.dataset.passportSupport='global';section.innerHTML=markup();const footer=document.querySelector('footer');footer?.parentNode?footer.parentNode.insertBefore(section,footer):document.body.appendChild(section);}setTimeout(maybeShowAdblockModal,350);};
