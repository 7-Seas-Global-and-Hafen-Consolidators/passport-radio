...
play.addEventListener('click',async()=>{if(!current?.stream)return;if(!audio.paused){audio.pause();play.textContent='▶ PLAY';status.textContent='PAUSADO · '+current.name;return}try{await audio.play();status.textContent='NO AR · '+current.name;play.textContent='❚❚ PAUSE'}catch{status.textContent='SINAL INDISPONÍVEL'}});
