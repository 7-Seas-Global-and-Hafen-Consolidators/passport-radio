/* PASSPORT HOME v6 — ambientes fora de #pp-feed. Motor de áudio intocado. */
(() => {
  "use strict";
  ...
  function stage() {
    const cards = [
      ...
      ["LIVE JAM", "Sessões ao vivo · sinal contínuo", "Continuous Signals™", "https://img.youtube.com/vi/w_1jWR76qCY/sddefault.jpg"]
  ...
  function nomad() {
    ...
    const cards = items.map((it) => {
      ...
      return `<article class="pp-nomad-card">${im}<span class="rv6-kicker">DOSSIÊ · ${esc(String(it.category || "").replace(/_/g, " "))}</span><h3><a href="${href(it.url)}">${esc(it.title)}</a></h3><p>${esc((it.deck || "").slice(0, 160))}</p><span class="rv6-meta">por <b>Mr. Nomad</b>${when ? " · " + esc(when) : ""}</span></article>`;
    }).join("");
  ...
  async function shop() {
    ...
    const cards = prods.map((p) => {
      const has = !!p.image;
      return `<a class="pp-product" href="loja.html?product=${esc(p.id)}&sku=${esc(p.sku || p.id)}">${has ? imgBlock(p.image, p.name) : `<div class="rv6-media" data-img="none"></div>`}<b>${esc(p.category || "LOJA")}</b><strong>${esc(p.name)}</strong><span class="pp-price">${money(p.price)}</span><span class="pp-installments">${esc(payLine)}</span><span class="pp-btn pp-btn--ink">VER PRODUTO →</span></a>`;
    }).join("");
