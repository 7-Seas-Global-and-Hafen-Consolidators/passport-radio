(() => {
  ...
  if (document.documentElement.dataset.passportRisingReactions === '3') return;
  document.documentElement.dataset.passportRisingReactions = '3';
  ...
  style.textContent = `
    ...
    .passport-rising-reaction__text{overflow:hidden;text-overflow:ellipsis}.passport-rising-reaction__name{font-weight:900}
    ...
    @media(max-width:700px){.passport-rising-reactions{right:7px;bottom:16px;width:min(220px,64vw);height:38vh}.passport-rising-reaction{max-width:210px;min-height:36px;padding:7px 9px;font-size:10px}}
  ...
  const activities = ['curtiu a Passport','entrou no World Dial','descobriu um Tunnel','está ouvindo agora','voltou para ouvir','está explorando a Passport','favoritou uma estação','entrou em Live & Rare','está no Continuous Signals','descobriu uma nova rádio'];
  ...
  const emitActivity = () => {
    ...
    text.append(name,document.createTextNode(` ${action}`)); bubble.append(text); decorate(bubble);
  };

  const emit = () => emitActivity();
  const schedule = () => {
    window.clearTimeout(timer); if(!running)return;
    timer=window.setTimeout(()=>{emitActivity();if(Math.random()<.20)window.setTimeout(()=>emitActivity(),300+Math.random()*500);schedule();},1500+Math.random()*2700);
  };
  ...
  window.PassportReactions=Object.freeze({emit,emitActivity,namesCount:names.length});
