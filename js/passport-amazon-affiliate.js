(() => {
  ...
  const installStyle=()=>{if(document.getElementById('passport-commercial-style'))return;const style=document.createElement('style');style.id='passport-commercial-style';style.textContent=`
    ...
    .passport-commercial-rail__brand,.passport-commercial-rail__copy{writing-mode:vertical-rl;transform:rotate(180deg);text-transform:uppercase}.passport-commercial-rail__brand{font-size:15px;line-height:1;font-weight:1000;letter-spacing:.07em}.passport-commercial-rail__copy{font-size:6px;line-height:1.1;font-weight:900;letter-spacing:.11em;opacity:.9}
    ...
    .passport-commercial-rail::after{content:'';position:absolute;inset:-45% -140%;z-index:-1;background:linear-gradient(105deg,transparent 42%,rgba(255,255,255,.32) 50%,transparent 58%);transform:translateX(-55%);animation:passportCommercialSweep 5.2s ease-in-out infinite}@keyframes passportCommercialSweep{0%,70%,100%{transform:translateX(-55%);opacity:0}76%{opacity:1}90%{transform:translateX(55%);opacity:1}94%{opacity:0}}
    ...
    /* Home hero hardening: the dark feature must never darken its own copy. */
    ...
    @media(max-width:760px){.passport-affiliate-legal{width:calc(100% - 20px);font-size:8px}}@media(prefers-reduced-motion:reduce){.passport-commercial-rail::after{animation:none}}@media print{.passport-commercial-rail,.passport-corner-ribbon,.passport-affiliate-legal{display:none!important}}
