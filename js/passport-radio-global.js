...
/* PASSPORT RADIO · LIVE & RARE · AUDIO-ONLY TUNNEL */
(() => {
  "use strict";
  ...
  const style=document.createElement("style");style.textContent=`
    ...
    .live-rare-engine{position:absolute!important;left:-10000px!important;top:-10000px!important;width:2px!important;height:2px!important;opacity:0!important;overflow:hidden!important;pointer-events:none!important}
    .live-rare-engine iframe{width:2px!important;height:2px!important;border:0!important;opacity:0!important;pointer-events:none!important}
  ...
  let index=Math.floor(Math.random()*catalog.length),player=null,ready=false,timer=null;const q=id=>document.getElementById(id),title=q("liveRareTitle"),meta=q("liveRareMeta"),status=q("liveRareStatus"),play=q("liveRarePlay"),progress=q("liveRareProgress"),tunnelTitle=q("liveRareTunnelTitle"),tunnelMeta=q("liveRareTunnelMeta");const item=()=>catalog[index];
  function sync(){const x=item();const name=`${x.artist} · ${x.title}`;title.textContent=name;meta.textContent=`${x.place} · ${x.year}`;tunnelTitle.textContent=name;tunnelMeta.textContent=`${x.place} · ${x.year}`;const top=q("currentTrackTitle");if(top)top.textContent=`${name} · ${x.year}`;const desc=q("currentTrackDescription");if(desc)desc.textContent="Passport Radio · Live & Rare";const bt=q("bottomTrackTitle");if(bt)bt.textContent=name;const bm=q("bottomTrackMeta");if(bm)bm.textContent="Passport Radio · Live & Rare"}
  ...
  function makePlayer(){if(player||!window.YT||!YT.Player)return;player=new YT.Player("passport-live-rare-engine",{width:"2",height:"2",playerVars:{playsinline:1,rel:0,controls:0,fs:0,disablekb:1,iv_load_policy:3},events:{onReady:()=>{ready=true;cue(false)},onStateChange:e=>{if(e.data===YT.PlayerState.PLAYING){status.textContent="ON AIR";play.textContent="Ⅱ";watch()}else if(e.data===YT.PlayerState.PAUSED||e.data===YT.PlayerState.CUED){status.textContent=e.data===YT.PlayerState.PAUSED?"PAUSED":"READY";play.textContent="▶";clearInterval(timer)}else if(e.data===YT.PlayerState.ENDED)move(1)},onError:()=>{status.textContent="NEXT";setTimeout(()=>move(1),500)}}})}
