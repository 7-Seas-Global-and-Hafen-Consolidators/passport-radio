...
const ids=new Set();for(const p of all){const k=p.sku||p.id;if(ids.has(k))E.push(`${k}: SKU duplicado`);ids.add(k);if(p.publish!==false&&(!(Number(p.price)>0)||!String(p.name||'').trim()))E.push(`${k}: publicável incompleto`);if(/"source_[^"]*"\s*:/i.test(JSON.stringify(p)))E.push(`${k}: source_* exposto`)}
console.log(`📦 Total: ${all.length} · publicáveis: ${all.filter(p=>p.publish!==false&&Number(p.price)>0).length}`);if(E.length){console.error(`❌ ${E.length} erros`);E.forEach(x=>console.error('  '+x));process.exit(1)}console.log('✅ Catálogo público válido · sem source_* · teto do manifesto 10x');
