...
Precision over coverage. This module is downstream of Editorial Engine and does
not alter RSS, audio, players, Home, Tunnels or other protected surfaces.
"""
...
UA = "Mozilla/5.0 PassportNewsroom/3.1 (+https://www.passportradio.online/)"
MAX_CANDIDATES = 8
# Configurable/testable: defaults remain conservative; CI/golden set records them.
CONFIDENCE_THRESHOLD = float(os.getenv("PASSPORT_NEWSROOM_CONFIDENCE_THRESHOLD", "0.80"))
SCORE_THRESHOLD = float(os.getenv("PASSPORT_NEWSROOM_SCORE_THRESHOLD", "0.68"))
AMBIGUITY_MARGIN = float(os.getenv("PASSPORT_NEWSROOM_AMBIGUITY_MARGIN", "0.12"))
...
def search_youtube_candidates(query: str, timeout: int = 15) -> list[str]:
    body = fetch_text("https://www.youtube.com/results?search_query=" + quote_plus(query), timeout)
    out, seen = [], set()
    for video_id in re.findall(r'"videoId":"([A-Za-z0-9_-]{11})"', body):
        if video_id in seen:
            continue
        seen.add(video_id); out.append(video_id)
...
def youtube_metadata(video_id: str) -> dict | None:
    ...
    date_match = re.search(r'"(?:publishDate|uploadDate)":"([^"T]+)', page)
    published_at = date_match.group(1) if date_match else ""
    return {"video_id": video_id, "title": clean(data.get("title")), "channel": clean(data.get("author_name")),
            "thumbnail_url": clean(data.get("thumbnail_url")), "published_at": published_at}
...
def evaluate_candidate(meta: dict, item: dict) -> dict:
    return evaluate(meta, item, score_threshold=SCORE_THRESHOLD, confidence_threshold=CONFIDENCE_THRESHOLD)
...
def choose_video(item: dict) -> dict:
    ...
    decision = {"query": query, "candidates": [], "selected_video": None, "score": 0.0, "confidence": 0.0,
                ...
                "thresholds": {"score": SCORE_THRESHOLD, "confidence": CONFIDENCE_THRESHOLD, "ambiguity_margin": AMBIGUITY_MARGIN}}
    if not entities(item):
        decision["reason_rejected"] = "missing_specific_entity"; return decision
    ids = search_youtube_candidates(query)
    if not ids:
        decision["reason_rejected"] = "no_candidates"; return decision
    evaluated = []
    for video_id in ids:
    ...
    if not viable:
        decision["reason_rejected"] = "all_candidates_vetoed"; return decision
    top = viable[0]; runner = viable[1] if len(viable) > 1 else None
    margin = top["confidence"] - (runner["confidence"] if runner else 0.0)
    decision.update({"score": top["score"], "confidence": top["confidence"], "positive_matches": top.get("positive_matches", []),
                     "penalties": top.get("penalties", []), "temporal_class": top.get("temporal_class"),
                     "event_specificity": top.get("event_specificity"), "authority_reason": top.get("authority_reason"),
                     "entity_evidence": top.get("entity_evidence")})
    if top["score"] < SCORE_THRESHOLD:
        decision["reason_rejected"] = "score_below_threshold"; return decision
    if top["confidence"] < CONFIDENCE_THRESHOLD:
        decision["reason_rejected"] = "confidence_below_threshold"; return decision
    if runner and margin < AMBIGUITY_MARGIN:
        decision["reason_rejected"] = "ambiguous_top_candidates"; return decision
    thumb = top.get("thumbnail_url") or f"https://i.ytimg.com/vi/{top['video_id']}/hqdefault.jpg"
    if not fetch_bytes_ok(thumb):
        decision["reason_rejected"] = "thumbnail_unavailable"; return decision
    decision["selected_video"] = top["video_id"]
    decision["reason_selected"] = "evidence_backed_editorial_match"
    return decision


def audit_path(run_id: str) -> Path:
    p = ROOT / "build" / "newsroom-audit"; p.mkdir(parents=True, exist_ok=True)
    return p / f"{run_id}.jsonl"


def append_audit(run_id: str, item: dict, url: str, decision: dict, outcome: str):
    payload = {"ts": datetime.now(timezone.utc).isoformat(), "run_id": run_id, "marker_version": MARKER_VERSION,
               "article": url, "title": clean(item.get("title")), "entities": entities(item), "query": decision.get("query"),
               "candidates": decision.get("candidates", []), "selected_video": decision.get("selected_video"),
               "score": decision.get("score"), "confidence": decision.get("confidence"),
               "positive_matches": decision.get("positive_matches", []), "penalties": decision.get("penalties", []),
               "reason_selected": decision.get("reason_selected", ""), "reason_rejected": decision.get("reason_rejected", ""),
               "temporal_class": decision.get("temporal_class"), "event_specificity": decision.get("event_specificity"),
               "authority_reason": decision.get("authority_reason"), "entity_evidence": decision.get("entity_evidence"),
               "thresholds": decision.get("thresholds", {}), "outcome": outcome}
...
def enrich_page(path: Path, item: dict, run_id: str, dry_run: bool = False) -> tuple[bool, str]:
    ...
    if BLOCK_START in original or "PASSPORT_NEWSROOM_ENRICHED_V2" in original:
        append_audit(run_id, item, url, {"query": "", "reason_rejected": "already_enriched"}, "SKIP"); return False, "SKIP"
    decision = choose_video(item); video_id = decision.get("selected_video")
    if not video_id:
        append_audit(run_id, item, url, decision, "NO_VIDEO"); return False, "NO_VIDEO"
    embed = f"https://www.youtube-nocookie.com/embed/{video_id}"; title = clean(item.get("title"))
    ...
    if needle not in original:
        decision.update({"selected_video": None, "reason_selected": "", "reason_rejected": "closing_anchor_missing"})
        append_audit(run_id, item, url, decision, "NO_VIDEO"); return False, "NO_VIDEO"
    ...
    if candidate == original or BLOCK_START not in candidate:
        decision.update({"selected_video": None, "reason_selected": "", "reason_rejected": "atomic_composition_failed"})
        append_audit(run_id, item, url, decision, "NO_VIDEO"); return False, "NO_VIDEO"
...
def main() -> int:
    ap = argparse.ArgumentParser(); ap.add_argument("--feed", default="data/editorial-feed.json"); ap.add_argument("--limit", type=int, default=5)
    ap.add_argument("--run-id", default="local"); ap.add_argument("--dry-run", action="store_true"); args = ap.parse_args()
    feed = json.loads((ROOT / args.feed).read_text("utf-8")); items = feed if isinstance(feed, list) else feed.get("items", [])
    ...
    for item in items[:max(1, args.limit)]:
        ...
        if did_change: changed.append(url)
    ...
    print(json.dumps({"status":"ok","mode":"dry-run" if args.dry_run else "canary","limit":args.limit,"enriched":len(changed),
                      "pages":changed,"no_video":no_video,"dry_run_embed":dry_embed,"skipped":skipped,
                      "audit":str(audit_path(args.run_id).relative_to(ROOT))}, ensure_ascii=False)); return 0
